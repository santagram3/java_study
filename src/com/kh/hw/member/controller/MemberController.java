...
public class MemberController {
    ...
    public boolean updatePassword(String id, String newPassword) {
        ...
        if (checkId(id)) {
            ...
            for (int i = 0; i < existMemberNum(); i++) {
                String beforNum = m[i].getPassword();
                m[i].setPassword(newPassword);
//                return String.format("바뀌기전 비밀번호 : %s 바뀌고 난뒤 비밀번호 : %s ", beforNum,newPassword);
                System.out.printf("바뀌기전 비밀번호 : %s 바뀌고 난뒤 비밀번호 : %s ", beforNum, newPassword);
                return true;
            }
        } else {
            return false;
